...
function setFactory(factoryId) {
    ...
    jQuery.ajax({
        ...
        success: function (data) {
            ...
            // click on the factory if admin user
            if (factoryId != 'all')
                $('#dynamic-factories')["0"].childNodes["0"].childNodes["0"].click();
        },
        async: false
    });
}
...
function switchToUserManager() {
    ...
    if (userObj.usertype == 'x') {
        window.location.href = "/hitech-smart-factory/UserController?action=getAdminUserManager";
    } else if (userObj.usertype == 'a') {
        window.location.href = "/hitech-smart-factory/UserController?action=getUserManager";
    }
